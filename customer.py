...
#   -> 아이디는 최소 5글자여야 하며, ex:@naver.com 
#   -> 첫글자는 영어소문자만 가능 두번째 글자부터 영어소문자, 숫자
#   -> 위의 패턴이 안맞는 경우 재입력 하도록 함
...
import re
custlist=[{'name': '고은1', 'gender': 'F', 'email': 'dnsrmd1@gmail.com', 'birthday': '1999'},
          {'name': '고은2', 'gender': 'M', 'email': 'dnsrmd2@gmail.com', 'birthday': '2000'},
          {'name': '고은3', 'gender': 'F', 'email': 'dnsrmd3@gmail.com', 'birthday': '1999'},
          {'name': '고은4', 'gender': 'F', 'email': 'dnsrmd4@gmail.com', 'birthday': '1999'}]
page=-1
...
while True:
    ...
    if choice=="I":  
        ...
        while True : # 74 ~ 77 반복 
            customer['gender']=input("성별 입력(M,F) : ").upper() # 대소문자 구분 없이 입력받아도 되도록 입력값을 대문자로 바꿔 인식
            if customer['gender'] in ('M','F') : # 'M','F' 둘 중 하나를 입력받으면 끝. 입력받을 때까지 반복
                break
        while True :
            regex=re.compile('[a-z][a-z0-9_]{4,}@[a-z]{2,}[.][a-z]{2,}')  # {2,} 2개 이상이어야함
            while True:
                ...
                if result :
                    break
                        ...
                        print('이메일을 형식에 맞춰 다시 입력해주세요.')
            check = 0  # 고객 정보가 중복 입력되지 않도록 이메일로 고객을 구분
            for i in custlist : 
                    if i['email'] == customer['email'] :
                        check = 1 # 이미 같은 이메일이 있으면 check 값을 1로 변경
                        break
            if check == 0 :
                    break  
            print("이미 가입된 이메일 주소입니다")
        while True :
            customer['birthday'] = input('출생년도 4자리 : ')
            if len(customer['birthday']) == 4 and customer['birthday'].isdigit() :
                break
        custlist.append(customer)
        page = len(custlist) - 1 # 0부터 들어갈거라
        print(custlist)
